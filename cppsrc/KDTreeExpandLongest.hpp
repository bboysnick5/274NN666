...
class KDTreeExpandLongest {
  public:
    ...
    // ----------------------------------------------------
    // Inserts the point pt into the KDTreeExpandLongest, associating it with
    ...
    // KDTreeExpandLongest. If the point does not exist, then it is added to the
    // KDTreeExpandLongest using the default value of ElemType as its key.
    ...
    // KDTreeExpandLongest nearest to v and returns the most common value
    ...
    // KDTreeExpandLongest nearest to p such that the farthest one in the set is
    // at least offset distance close to p than the rest of the points in the
    // tree. The forward iterator is passed in and filled and the end will be
    // returned.
  ...
  private:
    struct TreeNode {
        ...
        enum class dimEnum : std::size_t { LEFT_CHILD = N, NO_CHILDREN };
        ...
        TreeNode(std::uint8_t dim_to_expand, const PointND<value_type, N> &k,
                 const ElemType &obj)
            : right(nullptr), dim_to_expand(dim_to_expand), left(nullptr),
              key(k), object(obj) {}

        TreeNode(std::uint8_t dim_to_expand, PointND<value_type, N> &&k,
                 ElemType &&obj)
            : right(nullptr), dim_to_expand(dim_to_expand), left(nullptr),
    ...
    template <class RAI>
    void RangeCtorHelper(TreeNode *&, RAI, RAI,
                         std::array<value_type, N * 2> &);
    ...
    void NNValueHelper(TreeNode *, std::uint8_t dim,
                       const PointND<value_type, N> &, const ElemType *&,
                       value_type &) const;
    ...
    typename std::enable_if<
        std::is_same<typename std::iterator_traits<RAI>::iterator_category,
            ...
            !std::is_const<typename std::remove_pointer<
                typename std::iterator_traits<RAI>::pointer>::type>::value,
        int>::type>
KDTreeExpandLongest<FPType, N, ElemType, DT>::KDTreeExpandLongest(RAI begin,
                                                                  RAI end)
...
KDTreeExpandLongest<FPType, N, ElemType, DT>::operator=(
    const KDTreeExpandLongest &rhs) & {
    if (this != &rhs) {
        delete root;
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
KDTreeExpandLongest<FPType, N, ElemType, DT>::KDTreeExpandLongest(
    KDTreeExpandLongest &&rhs) noexcept
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
template <class RAI>
std::array<FPType, N * 2>
KDTreeExpandLongest<FPType, N, ElemType, DT>::computeInitBBox(RAI begin,
                                                              RAI end) {
    std::array<FPType, N * 2> bbox;
    std::generate(bbox.begin(), bbox.end(),
                  [lowHighToggle = 0lu,
                   lowHigh = std::array<FPType, 2>{
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
template <class RAI>
void KDTreeExpandLongest<FPType, N, ElemType, DT>::RangeCtorHelper(
    TreeNode *&ndPoolPtr, RAI begin, RAI end, std::array<FPType, N * 2> &bbox) {
    std::uint8_t dim = 0;
    FPType maxSpan = bbox[1] - bbox[0];
    for (std::size_t i = 1; i != N; ++i) {
        std::size_t bboxLowIdx = i * 2, bboxHighIdx = bboxLowIdx + 1;
    ...
    if (begin == median - 1) {
        // cur_ndPtr->dim_to_expand =
        // static_cast<std::underlying_type_t<typename
        // TreeNode::dimEnum>>(TreeNode::dimEnum::LEFT_CHILD);
        cur_ndPtr->left = ++ndPoolPtr;
        pool->construct(
            ndPoolPtr,
            static_cast<std::underlying_type_t<typename TreeNode::dimEnum>>(
                TreeNode::dimEnum::NO_CHILDREN),
            std::move(begin->key), std::move(begin->value));
    } else if (begin != median) {
        cur_ndPtr->left = ++ndPoolPtr;
        auto prev_high_on_dim = bbox[dim * 2 + 1];
        bbox[dim * 2 + 1] = cur_ndPtr->key[dim];
        // bbox[dim*2+1] = std::max_element(begin, median, [dim](const auto &p1,
        // const auto&p2){return p1.first[dim] < p2.first[dim];})->first[dim];
    ...
    if (median + 2 == end) {
        ...
        pool->construct(
            ...
            static_cast<std::underlying_type_t<typename TreeNode::dimEnum>>(
                TreeNode::dimEnum::NO_CHILDREN),
            std::move((median + 1)->key), std::move((median + 1)->value));
    } else if (median + 1 != end) {
        cur_ndPtr->right = ++ndPoolPtr;
        auto prev_low_on_dim = bbox[dim * 2];
        bbox[dim * 2] = cur_ndPtr->key[dim];
        // bbox[dim*2] = std::min_element(median + 1, end, [dim](const auto &p1,
        // const auto &p2){return p1.first[dim] < p2.first[dim];})->first[dim];
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
void KDTreeExpandLongest<FPType, N, ElemType, DT>::treeCopy(
    TreeNode *&thisNode, TreeNode *otherNode
    // TreeNode* ndPoolIt
) {
    if (otherNode) {
        if (thisNode) {
            ...
            //++ndPoolIt;
            thisNode = new TreeNode(otherNode->dim_to_expand, otherNode->key,
                                    otherNode->object);
        }
...
KDTreeExpandLongest<FPType, N, ElemType, DT>::dimension() const {
    return N;
}
...
KDTreeExpandLongest<FPType, N, ElemType, DT>::distType() const {
    return DT;
}
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
std::uint32_t
KDTreeExpandLongest<FPType, N, ElemType, DT>::heightHelper(TreeNode *n) const {
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
void KDTreeExpandLongest<FPType, N, ElemType, DT>::insert(
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
bool KDTreeExpandLongest<FPType, N, ElemType, DT>::contains(
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType &KDTreeExpandLongest<FPType, N, ElemType, DT>::operator[](
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType &
KDTreeExpandLongest<FPType, N, ElemType, DT>::at(const PointND<FPType, N> &pt) {
    return const_cast<ElemType &>(
        static_cast<const KDTreeExpandLongest &>(*this).at(pt));
}

template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
const ElemType &KDTreeExpandLongest<FPType, N, ElemType, DT>::at(
    const PointND<FPType, N> &pt) const {
...
KDTreeExpandLongest<FPType, N, ElemType, DT>::findNodePtr(
    const PointND<FPType, N> &pt) {
    return const_cast<TreeNode **>(
        static_cast<const KDTreeExpandLongest *>(this)->findNodePtr(pt));
}
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType KDTreeExpandLongest<FPType, N, ElemType, DT>::kNNValue(
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
void KDTreeExpandLongest<FPType, N, ElemType, DT>::kNNValueHelper(
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
template <class OutputIter>
OutputIter KDTreeExpandLongest<FPType, N, ElemType, DT>::NNsWithFence(
    const PointND<FPType, N> &pt, FPType fence, OutputIter returnIt) const {
    std::vector<
        std::tuple<FPType, const PointND<FPType, N> &, const ElemType &>>
        distPtElemTuple;
    distPtElemTuple.reserve(std::sqrt(treeSize));
    std::pair<FPType, const TreeNode *> st[32], *it = st;
    ...
    while (true) {
        FPType curDistSq =
            PointND<FPType, N>::template dist<def::DistType::kEucSq>(cur->key,
                                                                     pt);
        ...
        if (cur->right) {
            ...
            if (diff < 0.0) {
                new (it++) std::pair<FPType, const TreeNode *>(diff * diff,
                                                               cur->right);
                cur = cur->left;
            } else {
                new (it++)
                    std::pair<FPType, const TreeNode *>(diff * diff, cur->left);
                cur = cur->right;
            }
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType KDTreeExpandLongest<FPType, N, ElemType, DT>::NNValue(
    const PointND<FPType, N> &pt) const {
    ...
    /*
     static std::size_t totalNumNodesSearches = 0, numNNSearches = 0,
     totalTreeSize = 0; static std::uint8_t NumOfFullSearch = 0; std::size_t
     thisNumNodesSearches = 0; static bool logCondition; static constexpr
     std::size_t TREE_SIZE_LOWER_BOUND = 1200, TREE_SIZE_UPPER_BOUND = 60000000;
     logCondition = treeSize <= TREE_SIZE_UPPER_BOUND && treeSize >=
     TREE_SIZE_LOWER_BOUND;
    ...
    */
    ...
    /*
     ...
     << "\nAve num of nodes searched in each NN search: " <<
     totalNumNodesSearches/numNNSearches
     << "\n\nnum of full searches percentage: " <<
     numOfFullSearch*100.0/numNNSearches
     << "%\nSearched nodes over total num of nodes percentage: " <<
     totalNumNodesSearches*100.0/totalTreeSize << "%\n\n\n\n";
...
void KDTreeExpandLongest<FPType, N, ElemType, DT>::NNValueHelper(
    TreeNode *cur, std::uint8_t dim, const PointND<FPType, N> &pt,
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
FPType KDTreeExpandLongest<FPType, N, ElemType, DT>::branchMin(
