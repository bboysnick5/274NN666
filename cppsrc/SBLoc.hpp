...
inline bool SBLoc<FPType>::operator==(const SBLoc<FPType> &rhs) const {
    if (geo_pt == rhs.geo_pt)
        return true;
    return std::fabs(geo_pt[0] - rhs.geo_pt[0]) < static_cast<FPType>(0.000001) &&
...
namespace std {
    ...
    struct hash<SBLoc<FPType>> {
        std::size_t operator()(const SBLoc<FPType>& l) const {
            return (static_cast<std::size_t>((l.geo_pt[0] + 0.5*def::kMathPi<FPType>) * 1000000.0) << 20) +
                    static_cast<std::size_t>((l.geo_pt[1] + def::kMathPi<FPType>) * 1000000.0);
        }
    };
...
inline std::ostream& operator<<(std::ostream &os, const SBLoc<FPType> &l) {
    return os << "Lat: " << SBLoc<FPType>::toDegree(l.geo_pt[0]) << ", Lng: "
              << SBLoc<FPType>::toDegree(l.geo_pt[1]) << "\nCity: "
