...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT = def::DistType::kEuc>
class NoTree {
   public:
...
class KDTree {
   public:
        ...
        typename std::enable_if<
            std::is_same<
                typename std::iterator_traits<
                    typename std::remove_const_t<ConstRAI>>::iterator_category,
                std::random_access_iterator_tag>::value &&
                std::is_const<typename std::remove_pointer<
                    typename std::iterator_traits<ConstRAI>::pointer>::type>::
                    value,
            int>::type = 0>
    ...
    // specified value. If the element already existed in the tree, the new
    ...
    // KDTree. If the point does not exist, then it is added to the KDTree
    // using the default value of ElemType as its key.
    ...
    // ----------------------------------------------------
    // Returns a reference to the key associated with the point pt. If the
    // point is not in the tree, this function //throws an out_of_range
    // exception.
    ...
    // ----------------------------------------------------
    // Given a point p and a FPType offset, return a set of points in the
    // KDTree nearest to p such that the farthest one in the set is at least
    // offset distance close to p than the rest of the points in the tree.
    // The forward iterator is passed in and filled and the end will be
    // returned.
    template <class OutputIter>
    OutputIter NNsWithFence(const PointND<value_type, N> &, value_type,
                            OutputIter) const;
   ...
   private:
    ...
    template <class RAI>
    void RangeCtorHelper(TreeNode *&, std::uint8_t, RAI, RAI, RAI);

    // ----------------------------------------------------
    // Helper method for kNNValue search
    void kNNValueHelper(TreeNode *cur, std::uint8_t dim,
                        const PointND<value_type, N> &pt,
                        BoundedPQueue<ElemType, value_type> &bpq) const;

    // void NNsWithFenceHelper(TreeNode*, std::size_t, const PointND<FPType,
    // N>&, FPType,
    //                             std::vector<std::pair<FPType,
    //                          std::pair<PointND<FPType, N>, ElemType>>>&,
    //                          FPType&, FPType&) const;
    ...
    void NNValueHelper(TreeNode *, std::uint8_t, const PointND<value_type, N> &,
    ...
    void treeCopy(TreeNode *&thisNd,
                  TreeNode *otherNd  // TreeNode* ndPoolPtr
    );
    ...
    value_type branchMin(const PointND<value_type, N> &,
                         const PointND<value_type, N> &, std::size_t) const;
};
...
template <
    typename ConstRAI,
    typename std::enable_if<
        std::is_same<typename std::iterator_traits<typename std::remove_const_t<
                         ConstRAI>>::iterator_category,
                     std::random_access_iterator_tag>::value &&
            std::is_const<typename std::remove_pointer<
                typename std::iterator_traits<ConstRAI>::pointer>::type>::value,
        int>::type>
    ...
    typename std::enable_if<
        std::is_same<typename std::iterator_traits<RAI>::iterator_category,
            ...
            !std::is_const<typename std::remove_pointer<
                typename std::iterator_traits<RAI>::pointer>::type>::value,
        int>::type>
KDTree<FPType, N, ElemType, DT>::KDTree(RAI begin, RAI end)
    : treeSize(end - begin) {
    if (treeSize == 1) {
        root = new TreeNode(std::move(begin->key), std::move(begin->value));
    } else if (treeSize > 1) {
    ...
    actRecord st[static_cast<std::size_t>(log2(treeSize))];
    actRecord *it = st;
    bool hasChild = true;
    TreeNode** cur_ndPtr = &root;
    std::uint8_t dim = 0, next_dim = dim == N - 1 ? 0 : dim + 1;
    RAI thisBeginIt = begin, thisEndIt = end, median = thisBeginIt +
    (thisEndIt-thisBeginIt)/2; while (it != st || hasChild) { if (!hasChild)
    { hasChild = true;
            --it;
        ...
        std::nth_element(thisBeginIt, median, thisEndIt,
                         [=](const std::pair<PointND<FPType, N>, ElemType>&
    p1, const std::pair<PointND<FPType, N>, ElemType>& p2) { return
    p1.first[dim] < p2.first[dim];}); *cur_ndPtr = new
    TreeNode(std::move(median->first), std::move(median->second)); next_dim
    = dim == N - 1 ? 0 : dim + 1;

       if (median != thisBeginIt) {
            if (median + 1 != thisEndIt) {
                *it++ = {&(*cur_ndPtr)->left, next_dim, thisBeginIt,
    thisBeginIt
    + (median-thisBeginIt)/2, median}; cur_ndPtr = &(*cur_ndPtr)->right;
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
KDTree<FPType, N, ElemType, DT> &KDTree<FPType, N, ElemType, DT>::operator=(
    const KDTree &rhs) & {
    if (this != &rhs) {
        delete root;
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
template <class RAI>
void KDTree<FPType, N, ElemType, DT>::RangeCtorHelper(TreeNode *&cur_ndPtr,
                                                      std::uint8_t dim,
                                                      RAI begin, RAI median,
                                                      RAI end) {
    std::nth_element(begin, median, end, [=](const auto &nh1, const auto &nh2) {
        return nh1.key[dim] < nh2.key[dim];
    });
    cur_ndPtr = new TreeNode(std::move(median->key), std::move(median->value));
    std::uint8_t next_dim = dim == N - 1 ? 0 : dim + 1;
    if (begin == median - 1) {
        cur_ndPtr->left =
            new TreeNode(std::move(begin->key), std::move(begin->value));
    } else if (begin != median) {
        RangeCtorHelper(cur_ndPtr->left, next_dim, begin,
    ...
    if (median + 1 == end - 1) {
        ...
        RangeCtorHelper(cur_ndPtr->right, next_dim, median + 1,
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
constexpr std::uint8_t KDTree<FPType, N, ElemType, DT>::dimension() const {
    return N;
}

template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
typename def::DistType KDTree<FPType, N, ElemType, DT>::distType() const {
    return DT;
}
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
void KDTree<FPType, N, ElemType, DT>::insert(const PointND<FPType, N> &pt,
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
bool KDTree<FPType, N, ElemType, DT>::contains(
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType &KDTree<FPType, N, ElemType, DT>::operator[](
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
const ElemType &KDTree<FPType, N, ElemType, DT>::at(
    const PointND<FPType, N> &pt) const {
...
KDTree<FPType, N, ElemType, DT>::findNodePtr(const PointND<FPType, N> &pt) {
    return const_cast<TreeNode **>(
        static_cast<const KDTree *>(this)->findNodePtr(pt));
}

template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
typename KDTree<FPType, N, ElemType, DT>::TreeNode *const *
KDTree<FPType, N, ElemType, DT>::findNodePtr(
    const PointND<FPType, N> &pt) const {
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType KDTree<FPType, N, ElemType, DT>::kNNValue(const PointND<FPType, N> &pt,
                                                   std::size_t k) const {
    if (k == 1) return NNValue(pt);
    ...
    while (!bpq.empty()) {
        ...
        for (typename std::multimap<std::size_t, ElemType>::iterator it =
                 freqMap.begin();
             it != freqMap.end(); ++it) {
            if (it->second == elem) {
    ...
    /*
     -- This version requires ElemType to have its own hash function for
     best performance. But for the k size to be usually small, The
     difference between O(1) and O(lgk) and O(k) are hard to tell. So we use
     the uncommented version which does not require hash or comparison
     operators.
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
void KDTree<FPType, N, ElemType, DT>::kNNValueHelper(
    TreeNode *cur, std::uint8_t dim, const PointND<FPType, N> &pt,
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
template <class OutputIter>
OutputIter KDTree<FPType, N, ElemType, DT>::NNsWithFence(
    const PointND<FPType, N> &pt, FPType fence, OutputIter returnIt) const {
    /*
    std::vector<std::pair<FPType, std::pair<PointND<FPType, N>, ElemType>>>
    distKVPairs; distKVPairs.reserve(sqrt(treeSize)); FPType bestSq =
    std::numeric_limits<FPType>::max(), bestDiffSq =
    std::numeric_limits<FPType>::max(); NNsWithFenceHelper(root, 0, key,
    diff, distKVPairs, bestSq, bestDiffSq); for (const auto &p :
    distKVPairs) { if (p.first < bestDiffSq) *it++ = std::move(p.second);
    }
    return it;
    */

    std::vector<std::pair<
        FPType, std::pair<const PointND<FPType, N> &, const ElemType &>>>
        distKVPairs;
    distKVPairs.reserve(sqrt(treeSize));
    FPType bestDistSq = std::numeric_limits<FPType>::max(),
           bestDistDiffSq = std::numeric_limits<FPType>::max(), curDistSq,
           fenceSq = fence * fence;
    std::uint8_t dim = 0;
    ...
    while (it != st || hasNext) {
        if (!hasNext) {
            const auto &ar = *--it;
            if (!(ar.curDist < bestDistDiffSq && (cur = ar.cur))) continue;
            dim = ar.dim;
            hasNext = true;
        }
        curDistSq = PointND<FPType, N>::template dist<def::DistType::kEucSq>(
            cur->key, pt);
        if (curDistSq < bestDistDiffSq) {
            if (curDistSq < bestDistSq) {
                bestDistSq = curDistSq;
                bestDistDiffSq =
                    bestDistSq + fenceSq + 2 * fence * sqrt(bestDistSq);
            }
            distKVPairs.emplace_back(
                curDistSq,
                std::forward<
                    std::pair<const PointND<FPType, N> &, const ElemType &>>(
                    {cur->key, cur->object}));
        }
        ...
        dim = dim == N - 1 ? 0 : dim + 1;
        if (next) {
            *it++ = {curDistSq, next == cur->left ? cur->right : cur->left,
                     dim};
            cur = next;
        } else {
            if (curDistSq < bestDistDiffSq) {
...
/*
template <typename FPType, std::uint8_t N, typename ElemType,
typename def::DistType DT> void KDTree<FPType, N, ElemType, DT>::
NNsWithFenceHelper(TreeNode *cur, std::uint8_t dim, const PointND<FPType,
N>& pt, FPType diff, std::vector<std::pair<FPType, std::pair<PointND<FPType,
N>, ElemType>>> &distKVPairs, FPType& bestDistSq, FPType&bestDistDiffSq)
const { auto distSq = PointND<FPType, N>::template
dist<def::DistType::kEucSq>(cur->key, pt); if (distSq < bestDistDiffSq) { if
(distSq < bestDistSq) { bestDistSq = distSq; bestDistDiffSq = bestDistSq +
diff*diff + 2*diff*sqrt(bestDistSq);
        }
        distKVPairs.emplace_back(distSq, std::make_pair(cur->key,
cur->object));
    }
    std::uint8_t next_dim = dim + 1 < N ? dim + 1 : 0;
    ...
    if (next)
        NNsWithFenceHelper(next, next_dim, pt, diff, distKVPairs,
bestDistSq, bestDistDiffSq); if (thisDiff*thisDiff < bestDistDiffSq) { next
= next == cur->left ? cur->right : cur->left; if (next)
NNsWithFenceHelper(next, next_dim, pt, diff, distKVPairs, bestDistSq,
bestDistDiffSq);
    }
}*/
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType KDTree<FPType, N, ElemType, DT>::NNValue(
    const PointND<FPType, N> &pt) const {
    ...
    while (it != st || hasNext) {
        if (!hasNext) {
            const auto &ar = *--it;
            if (!(ar.curDist < bestDist && (cur = ar.cur))) continue;
            dim = ar.dim;
            hasNext = true;
        }
        next_dim = dim == N - 1 ? 0 : dim + 1;
        curDist = PointND<FPType, N>::template dist<def::DistType::kEucSq>(
            cur->key, pt);
        if (curDist < bestDist) {
            bestDist = curDist;
...
void KDTree<FPType, N, ElemType, DT>::NNValueHelper(
    TreeNode *cur, std::uint8_t dim, const PointND<FPType, N> &pt,
    ...
    if (next) NNValueHelper(next, next_dim, pt, bestValue, bestDist);
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
FPType KDTree<FPType, N, ElemType, DT>::branchMin(
