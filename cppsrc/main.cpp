...
void accuracyTestFromRefSolver(const std::vector<Point<dist_type, 2>> &testSearchLatLngPts,
    ...
    if (refLocs.empty()) {
        ...
        std::cout << "A total number of " << refLocs.size()
        ...
        for (int locIdx = 0; locIdx < refLocs.size() && std::chrono::steady_clock::now() - startTime < duration; ++locIdx) {
            ...
            std::chrono::duration<dist_type, std::micro> thisSearchDuration = std::chrono::steady_clock::now() - thisSearchStartTime;
            dist_type testDist, refDist;
            auto testPt = testSearchLatLngPts[locIdx];
            auto refLoc = refLocs[locIdx];
            if (testLoc != refLoc
                && (std::fabs((testDist = testLoc->havDist(testPt)) - (refDist = refLoc->havDist(testPt))))
                ...
                std::cout << "Test solver this one search time is: " << thisSearchDuration.count() << "μs" << std::endl
                          << (testDist < refDist ? "Test" : "Ref") << " dist is closer" << std::endl
                          << "Test Point: Lat: " << SBLoc<dist_type>::toDegree(testLat)
...
void timeNNSearch(const SBSolver<dist_type> &solver, std::vector<Point<dist_type, 2>> &testSearchLatLngPts,
    ...
    std::vector<std::chrono::duration<dist_type, std::micro>> perSearchTimeVec;
    std::chrono::duration<dist_type, std::micro> totalElapsedTime, maxOneSearchTime{}, minOneSearchTime(std::numeric_limits<dist_type>::max());
