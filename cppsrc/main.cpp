...
void AccuracyTestFromRefSolver(const std::vector<PointND<FPType, 2>> &test_lat_lng_pts,
    ...
    if (ref_locs.empty()) {
        ...
        for (std::size_t loc_idx = 0; loc_idx < test_lat_lng_pts.size() && (std::chrono::steady_clock::now() - start_time < duration); ++loc_idx) {
...
template <typename FPType>
void TimeBuild(std::span<const SBLoc<FPType>> loc_data_span, std::unique_ptr<SBSolver<FPType>>& solver, std::uint_fast8_t times = 10) {
...
int main(int argc, char** argv)
{
    ...
    //for (volatile int i = 0; i < 10; i++)
        std::thread([]() {}).detach();
    std::chrono::duration<double> elapsed_time_in_secs = std::chrono::steady_clock::now() - start;
