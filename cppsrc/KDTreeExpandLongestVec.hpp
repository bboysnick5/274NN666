...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
class KDTreeExpandLongestVec {
  public:
    ...
    // template <std::random_access_iterator RAI> requires
    // def::non_const_iterator<RAI> requires std::same_as<typename
    // std::iterator_traits<RAI>::value_type, typename
    // KDTreeExpandLongestVec<FPType, N, ElemType, DT>::node_type>
    template <
        typename RAI,
        std::enable_if_t<!std::is_const_v<typename std::remove_pointer_t<
                             typename std::iterator_traits<RAI>::pointer>>,
                         int> = 0>
    KDTreeExpandLongestVec(RAI, RAI);

    // template <std::random_access_iterator ConstRAI> requires
    // def::const_iterator<ConstRAI>
    //&& std::same_as<typename std::iter_value_t<ConstRAI>, typename
    // KDTreeExpandLongestVec<FPType, N, ElemType, DT>::node_type>
    template <
        typename ConstRAI,
        std::enable_if_t<std::is_const_v<typename std::remove_pointer_t<
                             typename std::iterator_traits<ConstRAI>::pointer>>,
                         int> = 0>
    ...
    // ----------------------------------------------------
    // Inserts the point pt into the KDTreeExpandLongestVec, associating it with
    // the specified value. If the element already existed in the tree, the new
    // value will overwrite the existing one.
    void insert(const PointND<FPType, N> &, const ElemType &);

    // ElemType& operator[](const PointND<FPType, N>& pt);
    // Usage: kd[v] = "Some Value";
    // ----------------------------------------------------
    // Returns a reference to the value associated with point pt in the
    // KDTreeExpandLongestVec. If the point does not exist, then it is added to
    // the KDTreeExpandLongestVec using the default value of ElemType as its
    // key.
    ElemType &operator[](const PointND<FPType, N> &pt);
    ...
    // ----------------------------------------------------
    // Given a point v and an integer k, finds the k points in the
    // KDTreeExpandLonge*stVec nearest to v and returns the most common value
    // associated with those points. In the event of a tie, one of the most
    // frequent value will be chosen.
    ElemType kNNValue(const PointND<FPType, N> &key, std::size_t k) const;

    // NdTypeOutIt NNsWithFence(const PointND<FPType, N>&, FPType, NdTypeOutIt)
    // const Usage: NdTypeOutIt end = kd.NNsWithFence(pt, 0.33, begin);
    // ----------------------------------------------------
    // Given a point p and a FPType fence in SQUARE, fill the output iterator
    // result with a collection of all the possible node_type structs whose
    // points as key are at least fence distance closer to p than the rest of
    // the points in the tree, and return the output iterator with final value
    // +1 position. The output iterator is passed in and filled with
    // point-element structs and the end will be returned.
    template <typename NdTypeOutIt>
    // requires std::output_iterator<NdTypeOutIt, typename
    // KDTreeExpandLongestVec<FPType, N, ElemType, DT>::node_type>
    NdTypeOutIt NNsWithFence(const PointND<FPType, N> &p, FPType fenseSq,
                             NdTypeOutIt result) const;
  ...
  private:
    ...
    template <typename RAI> void RangeCtorHelper(RAI, RAI);

    template <class RAI>
    void RangeCtorRecursion(MetaNode *&, ElemType *&, RAI, RAI,
                            std::array<FPType, N> &, std::array<FPType, N> &,
                            std::array<FPType, N> &);

    template <typename RAI>
    void MvConstructOneNdIncIter(MetaNode *&cur_nd, ElemType *&cur_elem,
                                 std::uint32_t right_idx, std::uint8_t dim,
                                 RAI nh_it);

    template <class ConstRAI>
    static std::tuple<std::array<FPType, N>, std::array<FPType, N>,
                      std::array<FPType, N>>
        ComputeInitBBoxHlSpread(ConstRAI, ConstRAI);

    // ----------------------------------------------------
    // Helper method for kNNValue search
    void kNNValueHelper(MetaNode *cur, std::uint8_t dim,
    ...
    template <
        typename def::DistType thisDt = DT,
    ...
    FPType branchMin(const PointND<FPType, N> &, const PointND<FPType, N> &,
                     std::uint8_t) const;
};
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
KDTreeExpandLongestVec<FPType, N, ElemType, DT>::KDTreeExpandLongestVec(
    const KDTreeExpandLongestVec &rhs)
    : size_(rhs.size_), cap_(rhs.cap_), height_(rhs.height_) {
    nd_arr_ = new MetaNode[size_];
    std::copy_n(rhs.nd_arr_, size_, nd_arr_);
    elem_arr_ = static_cast<ElemType *>(
        ::operator new(size_ * sizeof(ElemType), std::nothrow));
...
KDTreeExpandLongestVec<FPType, N, ElemType, DT>::operator=(
    const KDTreeExpandLongestVec &rhs) noexcept {
    if (this != &rhs) [[likely]] {
        DeAlloc();
        ...
        std::copy_n(rhs.nd_arr_, size_, nd_arr_);
        elem_arr_ =
            static_cast<ElemType *>(::operator new(size_ * sizeof(ElemType)));
        std::uninitialized_copy_n(rhs.elem_arr_, size_, elem_arr_);
    }
    return *this;
}
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
KDTreeExpandLongestVec<FPType, N, ElemType, DT>::KDTreeExpandLongestVec(
    KDTreeExpandLongestVec &&rhs) noexcept
    : nd_arr_(rhs.nd_arr_), elem_arr_(rhs.elem_arr_), size_(rhs.size_),
      cap_(rhs.cap_), height_(rhs.height_) {
    rhs.nd_arr_ = nullptr;
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
template <typename RAI>
void KDTreeExpandLongestVec<FPType, N, ElemType, DT>::MvConstructOneNdIncIter(
    MetaNode *&cur_nd, ElemType *&cur_elem, std::uint32_t right_idx,
    std::uint8_t dim, RAI nh_it) {
...
template <
    ...
    std::enable_if_t<std::is_const_v<typename std::remove_pointer_t<
                     ...
                     int>>
KDTreeExpandLongestVec<FPType, N, ElemType, DT>::KDTreeExpandLongestVec(
    ConstRAI cbegin, ConstRAI cend)
    : size_(static_cast<std::uint32_t>(cend - cbegin)), cap_(size_),
      height_(static_cast<std::uint32_t>(std::log2(size_) + 1)) {
...
KDTreeExpandLongestVec<FPType, N, ElemType, DT>::KDTreeExpandLongestVec(
    RAI begin, RAI end)
    : size_(static_cast<std::uint32_t>(end - begin)), cap_(size_),
      height_(static_cast<std::uint32_t>(std::log2(size_) + 1)) {
...
// template <std::random_access_iterator RAI>
template <typename RAI>
void KDTreeExpandLongestVec<FPType, N, ElemType, DT>::RangeCtorHelper(
    RAI data_begin, RAI data_end) {
    if (size_ == 0) [[unlikely]]
        return;
    nd_arr_ = new MetaNode[size_];
    elem_arr_ = static_cast<ElemType *>(
    ...
    auto [lows, highs, hl_spread] = ComputeInitBBoxHlSpread(
        std::as_const(data_begin), std::as_const(data_end));
    MetaNode *cur_nd = nd_arr_;
    ElemType *cur_elem = elem_arr_;
    RangeCtorRecursion(cur_nd, cur_elem, data_begin, data_end, lows, highs,
                       hl_spread);
    ...
    typename std::array<ActRecord, kMaxBalancedTreeHeight>::iterator ar_it =
    ar_stack.begin(); RAI this_begin = data_begin, this_end = data_end;

    auto RevertOneNode = [&ar_it](auto& highs_or_lows, auto& hl_spread, auto&
    highs, auto& lows) { auto& [dim_on_depth, end_idx_or_traced,
    prev_high_low_on_dim] = *ar_it; end_idx_or_traced = 0;
    ...
    while (true) {
        std::uint8_t dim =
    static_cast<std::uint8_t>(std::max_element(hl_spread.cbegin(),
    hl_spread.cend())
            - hl_spread.cbegin());
        ...
        std::nth_element(this_begin, median, this_end,
            [dim](const auto& nh1, const auto& nh2) {return nh1.key[dim] <
    nh2.key[dim]; }); MvConstructOneNdIncIter(cur_nd, cur_elem,
            static_cast<std::uint32_t>(cur_nd - nd_arr_ + left_branch_size + 1),
    dim, median);
        ...
        if (left_branch_size == 1) {
            ...
            if (RAI right_child_data_it = median + 1;
                ...
                MvConstructOneNdIncIter(cur_nd, cur_elem, 0, N,
    right_child_data_it); } else if (nd_ptr_this_iter->right_idx = 0; (ar_it -
    1)->end_idx_or_zero_as_traced
                ...
                ++this_end;
                // traceback: directly revert one left instead of set one right
    - revert one right
                --ar_it;
            ...
            auto& [dim_on_depth, end_idx_or_traced, prev_high_low_on_dim] =
    *ar_it++; util::CycleSwap(prev_high_low_on_dim, lows[dim_on_depth],
    highs[dim_on_depth]); hl_spread[dim_on_depth] = highs[dim_on_depth] -
    lows[dim_on_depth]; this_begin = this_end + 1; this_end = data_begin +
    end_idx_or_traced; end_idx_or_traced = 0; } else { *ar_it++ = {dim,
    static_cast<std::uint32_t>(this_end - data_begin), highs[dim]}; highs[dim] =
    nd_ptr_this_iter->key[dim]; hl_spread[dim] = highs[dim] - lows[dim];
            this_end = median;
        }
    } */
    ...
    std::array<ActRecord, kMaxBalancedTreeHeight> ar_stack;
    typename std::array<ActRecord, kMaxBalancedTreeHeight>::iterator ar_it =
    ar_stack.begin(); typename std::array<ActRecord,
    kMaxBalancedTreeHeight>::iterator leaf_prt_ar_it = ar_stack.begin() +
    (height_ > 2 ? height_ - 2 : 0); RAI this_begin = data_begin, this_end =
    data_end;

    auto RevertOneNode = [&ar_it](auto& highs_or_lows, auto& hl_spread, auto&
    highs, auto& lows) { auto &[dim_on_depth, end_idx_or_traced,
    prev_high_low_on_dim] = *ar_it; highs_or_lows[dim_on_depth] =
    prev_high_low_on_dim; hl_spread[dim_on_depth] = highs[dim_on_depth] -
    lows[dim_on_depth];
    };
    ...
    hl_spread.cend())
            ...
            std::nth_element(this_begin, median, this_end,
                [dim](const auto& nh1, const auto& nh2) {return nh1.key[dim] <
    nh2.key[dim]; }); MvConstructOneNdIncIter(cur_nd, cur_elem,
                static_cast<std::uint32_t>(cur_nd - nd_arr_ + left_subtree_size
    + 1), dim, median); *ar_it = {dim, static_cast<std::uint32_t>(this_end -
    data_begin), std::exchange(highs[dim], (cur_nd-1)->key[dim])};
    ...
    hl_spread.cend())
            ...
            if (median + 1 == this_end) {
                ...
                util::Sort3(this_begin, median, median+1, [dim](const auto&
    nh1, const auto& nh2) {return nh1.key[dim] < nh2.key[dim]; });
                MvConstructOneNdIncIter(cur_nd, cur_elem,
    static_cast<std::uint32_t>(cur_nd - nd_arr_ + 2), dim, median);
                MvConstructOneNdIncIter(cur_nd, cur_elem, 0, N, this_begin);
    ...
    *(--ar_it); util::CycleSwap(prev_high_low_on_dim, lows[dim_on_depth],
    ...
    DEBUG_PRINT:
        std::for_each_n(nd_arr_, size_, [](const MetaNode& nd){
            std::cout << "right_idx: " << nd.right_idx << '\t' <<
    "dim_to_expand: " << static_cast<std::uint32_t>(nd.dim_to_expand)  << '\t';
            std::cout << "pt coord: ";
            std::copy(nd.key.cbegin(), nd.key.cend(),
    std::ostream_iterator<FPType>(std::cout,",")); std::cout << '\n';
        }); */
}
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
template <class ConstRAI>
std::tuple<std::array<FPType, N>, std::array<FPType, N>, std::array<FPType, N>>
KDTreeExpandLongestVec<FPType, N, ElemType, DT>::ComputeInitBBoxHlSpread(
    ConstRAI cbegin, ConstRAI cend) {
    ...
    std::transform(highs.cbegin(), highs.cend(), lows.cbegin(),
                   hl_spread.begin(), std::minus<FPType>());
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
template <class RAI>
void KDTreeExpandLongestVec<FPType, N, ElemType, DT>::RangeCtorRecursion(
    MetaNode *&cur_nd, ElemType *&cur_elem, RAI begin, RAI end,
    std::array<FPType, N> &lows, std::array<FPType, N> &highs,
    std::array<FPType, N> &hl_spread) {
    std::uint8_t dim = static_cast<std::uint8_t>(
        std::distance(hl_spread.cbegin(),
                      std::max_element(hl_spread.cbegin(), hl_spread.cend())));
    MetaNode *nd_ptr_this_iter = cur_nd;
    std::ptrdiff_t left_branch_size = (end - begin) / 2;
    RAI median = begin + left_branch_size;
    std::nth_element(begin, median, end,
                     [dim](const auto &nh1, const auto &nh2) {
                         return nh1.key[dim] < nh2.key[dim];
                     });
    MvConstructOneNdIncIter(
        cur_nd, cur_elem,
        static_cast<std::uint32_t>(cur_nd - nd_arr_ + left_branch_size + 1),
        dim, median);
    ...
    if (left_branch_size == 1) {
        ...
        FPType prev_high_on_dim =
            std::exchange(highs[dim], nd_ptr_this_iter->key[dim]);
        hl_spread[dim] = highs[dim] - lows[dim];
        RangeCtorRecursion(cur_nd, cur_elem, begin, median, lows, highs,
                           hl_spread);
    ...
    if (median + 2 == end) {
        ...
        FPType prev_low_on_dim =
            std::exchange(lows[dim], nd_ptr_this_iter->key[dim]);
        hl_spread[dim] = highs[dim] - lows[dim];
        RangeCtorRecursion(cur_nd, cur_elem, median + 1, end, lows, highs,
                           hl_spread);
    ...
    /*
    std::uint8_t dim =
    static_cast<std::uint8_t>(std::distance(hl_spread.cbegin(),
    std::max_element(hl_spread.cbegin(), hl_spread.cend()))); MetaNode*
    nd_ptr_this_iter = cur_nd; std::ptrdiff_t left_branch_size = (end -
    begin)/2; RAI median = begin + left_branch_size; std::nth_element(begin,
    median, end, [dim](const auto& p1, const auto& p2) {return p1.key[dim] <
    p2.key[dim];});
    //oneapi::dpl::nth_element(oneapi::dpl::execution::par_unseq, begin, median,
    end, [dim](const auto& p1, const auto& p2) {return p1.key[dim] <
    p2.key[dim]; });

    *cur_nd++ = {static_cast<std::uint32_t>(cur_nd - nd_arr_ + left_branch_size
    + 1), dim, std::move(median->key)}; new (cur_elem++) ElemType
    ...
    if (left_branch_size == 1) {
        *cur_nd++ = {0, N, std::move(begin->key)};
        ...
        FPType prev_high_on_dim = std::exchange(highs[dim],
    nd_ptr_this_iter->key[dim]); hl_spread[dim] = highs[dim] - lows[dim];
        RangeCtorRecursion(cur_nd, cur_elem, begin, median, lows, highs,
    hl_spread); highs[dim] = prev_high_on_dim; hl_spread[dim] = prev_high_on_dim
    - lows[dim];
    }
    ...
    if (std::distance(median, end) == 2) {
        *cur_nd++ = {0, N, std::move((median+1)->key)};
        ...
        FPType prev_low_on_dim = std::exchange(lows[dim],
    nd_ptr_this_iter->key[dim]); hl_spread[dim] = highs[dim] - lows[dim];
        RangeCtorRecursion(cur_nd, cur_elem, median+1, end, lows, highs,
    hl_spread); lows[dim] = prev_low_on_dim; hl_spread[dim] = highs[dim] -
    prev_low_on_dim;
    } */
}
...
KDTreeExpandLongestVec<FPType, N, ElemType, DT>::dimension() const {
    return N;
}

template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
typename def::DistType
KDTreeExpandLongestVec<FPType, N, ElemType, DT>::distType() const {
    return DT;
}
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
bool KDTreeExpandLongestVec<FPType, N, ElemType, DT>::operator==(
    const KDTreeExpandLongestVec &rhs) const {
    if (size_ != rhs.size_)
        return false;
    return std::equal(elem_arr_, elem_arr_ + size_, rhs.elem_arr_) &&
           std::equal(nd_arr_, nd_arr_ + size_, rhs.nd_arr_);
}
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
void KDTreeExpandLongestVec<FPType, N, ElemType, DT>::insert(
    const PointND<FPType, N> &pt, const ElemType &value) {
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
bool KDTreeExpandLongestVec<FPType, N, ElemType, DT>::contains(
    const PointND<FPType, N> &pt) const {
    return *findNodePtr(pt) != nullptr;
}

template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType &KDTreeExpandLongestVec<FPType, N, ElemType, DT>::operator[](
    const PointND<FPType, N> &pt) {
    MetaNode **ndPtr = findNodePtr(pt);
    if (!*ndPtr) {
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType &KDTreeExpandLongestVec<FPType, N, ElemType, DT>::at(
    const PointND<FPType, N> &pt) {
    return const_cast<ElemType &>(
        static_cast<const KDTreeExpandLongestVec &>(*this).at(pt));
}

template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
const ElemType &KDTreeExpandLongestVec<FPType, N, ElemType, DT>::at(
    const PointND<FPType, N> &pt) const {
...
KDTreeExpandLongestVec<FPType, N, ElemType, DT>::findNodePtr(
    const PointND<FPType, N> &pt) {
    return const_cast<MetaNode **>(
        static_cast<const KDTreeExpandLongestVec *>(this)->findNodePtr(pt));
}
...
KDTreeExpandLongestVec<FPType, N, ElemType, DT>::findNodePtr(
    ...
    MetaNode *const *n;
    for (std::uint8_t dim = 0; *n && (*n)->key != pt;
         dim = dim == N - 1 ? 0 : dim + 1) {
    }
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType KDTreeExpandLongestVec<FPType, N, ElemType, DT>::kNNValue(
    const PointND<FPType, N> &pt, std::size_t k) const {
    // if (empty())
    //     return ElemType();
    if (k == 1)
        return NNValue(pt);
    ...
    while (!bpq.empty()) {
        ...
        for (typename std::multimap<std::uint32_t, ElemType>::iterator it =
                 freqMap.begin();
             it != freqMap.end(); ++it) {
            if (it->second == elem) {
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
void KDTreeExpandLongestVec<FPType, N, ElemType, DT>::kNNValueHelper(
    MetaNode *cur, std::uint8_t dim, const PointND<FPType, N> &pt,
    BoundedPQueue<ElemType, FPType> &bpq) const {
    bpq.enqueue(cur->object,
                PointND<FPType, N>::template dist<DT>(cur->key, pt));
...
// requires std::output_iterator<NdTypeOutIt, typename
// KDTreeExpandLongestVec<FPType, N, ElemType, DT>::node_type>
NdTypeOutIt KDTreeExpandLongestVec<FPType, N, ElemType, DT>::NNsWithFence(
    const PointND<FPType, N> &pt, FPType fence_sq,
    NdTypeOutIt pe_out_it) const {
    struct ActRecord {
    ...
    FPType cur_dist_sq,
        best_dist_sq = PointND<FPType, N>::template dist<def::DistType::kEucSq>(
            cur_nd->key, pt);
    FPType best_dist_plus_fence_sq =
        best_dist_sq + fence_sq + FPType(2.0) * sqrt(fence_sq * best_dist_sq);
    ...
    typename std::array<DistPtElem, kMaxDpeSizeOnStack>::iterator
    ...
    *result_dpe_arr_it++ = {best_dist_sq, &nd_arr_->key, elem_arr_};
    ...
    auto CheckOneNd = [&](const auto &nd) {
        cur_dist_sq = PointND<FPType, N>::template dist<def::DistType::kEucSq>(
            nd->key, pt);
        if (cur_dist_sq < best_dist_plus_fence_sq) {
            if (cur_dist_sq < best_dist_sq) {
                best_dist_sq = cur_dist_sq;
                best_dist_plus_fence_sq =
                    best_dist_sq + fence_sq +
                    FPType(2.0) * sqrt(fence_sq * best_dist_sq);
            }
            *result_dpe_arr_it++ = {cur_dist_sq, &nd->key,
                                    &elem_arr_[nd - nd_arr_]};
            if (result_dpe_arr_it == result_dpe_arr.end()) [[unlikely]] {
                result_dpe_arr_it = result_dpe_arr.begin();
                result_dpe_vec.insert(result_dpe_vec.end(),
                                      result_dpe_arr.begin(),
                                      result_dpe_arr.end());
            }
        }
    };
    ...
    while (true) {
        if ((right_idx = cur_nd->right_idx)) {
            ++cur_nd_idx;
            FPType diff =
                pt[cur_nd->dim_to_expand] - cur_nd->key[cur_nd->dim_to_expand];
            if (diff < 0.0) {
            ...
            do {
                if (ar_it == ar_stack) [[unlikely]] {
                    auto filter_transform_dpe_arr_to_pe_arr =
                        [&pe_out_it,
                         best_dist_plus_fence_sq](const auto &dpe) mutable {
                            if (dpe.dist_sq < best_dist_plus_fence_sq)
                                *pe_out_it++ = {*(dpe.pt), *(dpe.elem)};
                        };
                    std::for_each(std::make_reverse_iterator(result_dpe_arr_it),
                                  result_dpe_arr.rend(),
                                  filter_transform_dpe_arr_to_pe_arr);
                    std::for_each(result_dpe_vec.rbegin(),
                                  result_dpe_vec.rend(),
                                  filter_transform_dpe_arr_to_pe_arr);
                    return pe_out_it;
                }
...
std::fill(pt_ndPtr_arr_size.begin(), pt_ndPtr_arr_size.end(),
kMaxBalancedTreeHeight); auto pt_ndPtr_soa_view =
llama::allocView(llama::mapping::SoA<llama::ArrayDims<N>,
PtNodePtr>(pt_ndPtr_arr_size)); pt_ndPtr_soa_view(0ull, 0ull,
0ull)(ns::NodePtr{}) = nd_arr_;
*/

// static std::size_t search_times = 0, check_count = 0;
/*
template <typename FPType, std::uint8_t N, typename ElemType,
typename def::DistType DT> ElemType KDTreeExpandLongestVec<FPType, N, ElemType,
DT>::NNValue(const PointND<FPType, N> &search_pt) const { struct ActRecord {
        FPType diff_on_dim_sq;
        std::uint32_t on_stack_idx;
    };
    std::array<ActRecord, kMaxBalancedTreeHeight> ar_stack;
    typename std::array<ActRecord, kMaxBalancedTreeHeight>::iterator
        ar_it = ar_stack.begin(), leaf_prt_ar_it = ar_stack.begin() + (height_ >
2 ? height_ - 2 : 0);
    // BIG ASSUMPTION TREE IS BALANCED, otherwise stackoverflow
    const MetaNode *cur_nd = nd_arr_, *best_node = nd_arr_;
    FPType best_dist_sq = PointND<FPType, N>::template
dist<def::DistType::kEucSq>(cur_nd->key, search_pt);

    auto CheckOneNd = [&](const auto& nd) {
        if (FPType cur_dist_sq = PointND<FPType, N>::template
dist<def::DistType::kEucSq>(nd->key, search_pt); cur_dist_sq < best_dist_sq) {
            best_dist_sq = cur_dist_sq;
            best_node = nd;
        }
        //check_count++;
    ...
    while (true) {
        if (std::uint32_t right_idx = cur_nd->right_idx) {
            ...
            if (diff < 0.0) {
                ...
                *ar_it++ = {diff*diff,
   static_cast<std::uint32_t>(cur_nd-nd_arr_+1)}; cur_nd = nd_arr_ + right_idx;
            }
            CheckOneNd(cur_nd);
        } else {
            if (cur_nd->dim_to_expand != N)
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType KDTreeExpandLongestVec<FPType, N, ElemType, DT>::NNValue(
    const PointND<FPType, N> &search_pt) const {

    const MetaNode *cur_nd = nd_arr_;
    FPType best_dist_sq =
        PointND<FPType, N>::template dist<def::DistType::kEucSq>(cur_nd->key,
                                                                 search_pt);
    ...
    auto CheckOneNd = [&](const auto &nd, const std::uint32_t &cur_nd_idx) {
        if (FPType cur_dist_sq =
                PointND<FPType, N>::template dist<def::DistType::kEucSq>(
                    nd->key, search_pt);
    ...
    while (true) {
        if ((right_idx = cur_nd->right_idx)) {
            ++cur_nd_idx;
            FPType diff = search_pt[cur_nd->dim_to_expand] -
                          cur_nd->key[cur_nd->dim_to_expand];
            if (diff < 0.0) {
                *ar_it = {diff * diff, right_idx};
                ++cur_nd;
            } else {
...
DT>::NNValue(const PointND<FPType, N>& search_pt) const { struct ActRecord {
    ...
    std::array<ActRecord, kMaxBalancedTreeHeight> ar_stack;
    typename std::array<ActRecord, kMaxBalancedTreeHeight>::iterator
        ar_it = ar_stack.begin() + 1, cand_nd_ar_it, prev_ar_it =
ar_stack.begin();
    ...
    while (true) {
        if (std::uint32_t right_idx = cur_nd->right_idx) {
            ...
            if (diff < 0.0) {
                *ar_it++ = {diff*diff, right_idx,
static_cast<std::uint32_t>(++cur_nd-nd_arr_)}; } else { *ar_it++ = {diff*diff,
static_cast<std::uint32_t>(cur_nd-nd_arr_+1), right_idx}; cur_nd = nd_arr_ +
right_idx;
            }
        } else {
            cand_nd_ar_it = ar_it;
            if (cur_nd->dim_to_expand != N)
                cand_nd_ar_it++->cand_nd_idx =
static_cast<std::uint32_t>(++cur_nd - nd_arr_);
            std::for_each(std::make_reverse_iterator(cand_nd_ar_it),
std::make_reverse_iterator(prev_ar_it), [&](const auto& ar) { if (FPType
cur_dist_sq = PointND<FPType, N>::template dist<def::DistType::kEucSq>((nd_arr_
+ ar.cand_nd_idx)->key, search_pt); cur_dist_sq < best_dist_sq) { best_dist_sq =
cur_dist_sq; best_nd_idx = ar.cand_nd_idx;
                }
            });
            do {
...
DT>::NNValue(const PointND<FPType, N>& search_pt) const { struct ActRecord {
    ...
    typename std::array<ActRecord, kMaxBalancedTreeHeight>::iterator ar_it =
ar_stack.begin(); typename std::array<ActRecord,
kMaxBalancedTreeHeight>::iterator cand_nd_ar_it = ar_stack.begin(); constexpr
std::uint32_t num_lanes = 32/sizeof(FPType);
    ...
    FPType best_dist_sq = PointND<FPType, N>::template
dist<def::DistType::kEucSq>(cur_nd->key, search_pt); Eigen::Matrix<FPType, 1,
N> search_pt_v = Eigen::Map<Eigen::Matrix<FPType, 1,
N>>(const_cast<FPType*>(search_pt.DataArray().data())); while (true) { if
(std::uint32_t right_idx = cur_nd->right_idx) { std::uint8_t dim =
cur_nd->dim_to_expand; FPType diff = search_pt[dim] - cur_nd->key[dim];
            ar_it->diff_on_dim_sq = diff * diff;
            if (diff < 0.0) {
                ar_it++->on_stack_nd_idx = right_idx;
                ++cur_nd;
            } else {
                ar_it++->on_stack_nd_idx = static_cast<std::uint32_t>(cur_nd -
nd_arr_ + 1); cur_nd = nd_arr_ + right_idx;
            }
        } else if (cur_nd++->dim_to_expand == N) {
            if (lane_idx != 0) {
                if (FPType best_dist_sq_this_iter =
                    (cand_pts_matrix(Eigen::seq(0, lane_idx - 1),
Eigen::all).rowwise() - search_pt_v)
        ...
        cand_pts_matrix.row(lane_idx++) = Eigen::Map<Eigen::Matrix<FPType, 1,
N>>(const_cast<FPType*>(cur_nd->key.DataArray().data()));
        cand_nd_ar_it++->cand_nd_idx = static_cast<std::uint32_t>(cur_nd -
nd_arr_); if (lane_idx == num_lanes) { if (FPType best_dist_sq_this_iter =
(cand_pts_matrix.rowwise() - search_pt_v)
                .rowwise().squaredNorm().minCoeff(&lane_idx);
                best_dist_sq_this_iter < best_dist_sq) {
                best_dist_sq = best_dist_sq_this_iter;
                best_idx = ar_stack[lane_idx].cand_nd_idx;
...
ar_stack.begin();
    ...
    Eigen::Matrix<FPType, 1, N> search_pt_v = Eigen::Map<Eigen::Matrix<FPType,
1, N>>(const_cast<FPType*>(search_pt.DataArray().data())); Eigen::Matrix<FPType,
kMaxBalancedTreeHeight, N> cand_pts_matrix; std::uint32_t cand_pts_matrix_idx =
0;
...
static_cast<std::uint32_t>(++cur_nd - nd_arr_);
            std::for_each(std::make_reverse_iterator(cand_nd_ar_it),
std::make_reverse_iterator(prev_ar_it), [&](const auto& ar) {
                cand_pts_matrix.row(cand_pts_matrix_idx++) =
Eigen::Map<Eigen::Matrix<FPType, 1, N>>(const_cast<FPType*>((nd_arr_ +
ar.cand_nd_idx)->key.DataArray().data()));
            });
            if (FPType best_dist_sq_this_iter =
                (cand_pts_matrix(Eigen::seq(0, cand_pts_matrix_idx - 1),
Eigen::all).rowwise() - search_pt_v)
                .rowwise().squaredNorm().minCoeff(&cand_pts_matrix_idx);
                best_dist_sq_this_iter < best_dist_sq) {
                best_dist_sq = best_dist_sq_this_iter;
                best_nd_idx = (std::make_reverse_iterator(cand_nd_ar_it) +
cand_pts_matrix_idx)->cand_nd_idx;
            }
...
DT>::NNValue(const PointND<FPType, N> &search_pt) const { struct ActRecord {
    ...
    typename std::array<ActRecord, kMaxBalancedTreeHeight>::iterator ar_it =
ar_stack.begin(); typename std::array<ActRecord,
kMaxBalancedTreeHeight>::iterator cand_nd_ar_it = ar_stack.begin();
...
nd_arr_ + 1); cur_nd = nd_arr_ + right_idx;
            ...
            cand_pts_matrix.row(cand_pts_matrix_idx++) =
Eigen::Map<Eigen::Matrix<FPType, 1,
N>>(const_cast<FPType*>(cur_nd->key.DataArray().data()));
            cand_nd_ar_it++->cand_nd_idx = static_cast<std::uint32_t>(cur_nd -
nd_arr_); } else { if (cur_nd++->dim_to_expand != N) {
                cand_pts_matrix.row(cand_pts_matrix_idx++) =
Eigen::Map<Eigen::Matrix<FPType, 1,
N>>(const_cast<FPType*>(cur_nd->key.DataArray().data()));
                cand_nd_ar_it++->cand_nd_idx = static_cast<std::uint32_t>(cur_nd
...
nd_arr_ + 1); cur_nd = nd_arr_ + right_idx;
            ...
            for (std::int32_t cur_dim = N - 1; cur_dim >= 0; --cur_dim)
                cand_pts_matrix[cur_dim*kMaxBalancedTreeHeight +
cand_pts_matrix_idx] = cur_nd->key[cur_dim];
            ++cand_pts_matrix_idx;
            cand_nd_ar_it++->cand_nd_idx = static_cast<std::uint32_t>(cur_nd -
nd_arr_); } else { if (cur_nd++->dim_to_expand != N) { for (std::int32_t cur_dim
= N - 1; cur_dim >= 0; --cur_dim) cand_pts_matrix[cur_dim*kMaxBalancedTreeHeight
+ cand_pts_matrix_idx] = cur_nd->key[cur_dim];
                ++cand_pts_matrix_idx;
                cand_nd_ar_it++->cand_nd_idx = static_cast<std::uint32_t>(cur_nd
- nd_arr_);
            ...
            for (; cand_pts_matrix_idx >= 0; --cand_pts_matrix_idx) {
                ...
                for (std::int32_t cur_dim = N - 1; cur_dim >= 0; --cur_dim) {
                    auto& cand_pt_val_on_dim =
cand_pts_matrix[cur_dim*kMaxBalancedTreeHeight + cand_pts_matrix_idx];
...
void KDTreeExpandLongestVec<FPType, N, ElemType, DT>::NNValueHelper(
    MetaNode *cur, std::uint8_t dim, const PointND<FPType, N> &pt,
    const ElemType *&bestValue, FPType &bestDist) const {
    FPType curDist =
        PointND<FPType, N>::template dist<def::DistType::kEucSq>(cur->key, pt);
    if (curDist < bestDist) {
        bestDist = curDist;
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
FPType KDTreeExpandLongestVec<FPType, N, ElemType, DT>::branchMin(
    const PointND<FPType, N> &trPt, const PointND<FPType, N> &searchPt,
    std::uint8_t dim) const {
    switch (DT) {
