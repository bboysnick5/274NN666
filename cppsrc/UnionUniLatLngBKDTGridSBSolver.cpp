...
UnionCell(size_t maxCacheVecSize, const std::vector<std::pair<Point<3>,
...
BitCell(size_t maxCacheVecSize,
        const std::vector<std::pair<Point<3>, const SBLoc*>> &bufVec) {
    ...
    if (size == 1) {
        ...
        std::pair<Point<3>, const SBLoc*> *cacheLocs = static_cast<std::pair<Point<3>, const SBLoc*>*>(::operator new(size*sizeof(std::pair<Point<3>, const SBLoc*>), std::nothrow));
        std::uninitialized_move(bufVec.cbegin(), bufVec.cend(), cacheLocs);
        //std::pair<Point<3>, const SBLoc*> *cacheLocs = new std::pair<Point<3>, const SBLoc*>[size];
        //std::copy(bufVec.cbegin(), bufVec.cend(), cacheLocs);
...
template <template <size_t, class, typename Point<3>::DistType> class KDTType>
const std::pair<Point<3>, const SBLoc*>* UnionUniLatLngBKDTGridSBSolver<KDTType>::BitCell::getLocPairs() const {
    return reinterpret_cast<std::pair<Point<3>, const SBLoc*>*>(static_cast<intptr_t>(ptr << 16) >> 16);
}
    ...
    if (cell.size() == 1) {
        ...
        return std::min_element(cell.getLocPairs(), cell.getLocPairs() + cell.size(),
                                [&](const auto& p1, const auto& p2){ return Point<3>::template
                                    dist<Point<3>::DistType::EUCSQ>(p1.first, p) <
