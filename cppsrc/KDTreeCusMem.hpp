...
class KDTreeCusMem {
  public:
    ...
    // ----------------------------------------------------
    // Inserts the point pt into the KDTreeCusMem, associating it with the
    ...
    // KDTreeCusMem. If the point does not exist, then it is added to the
    // KDTreeCusMem using the default value of ElemType as its key.
    ...
    // KDTreeCusMem nearest to p such that the farthest one in the set is at
    // least offset distance close to p than the rest of the points in the tree.
...
template <
    ...
    typename std::enable_if<
        ...
        int>::type>
KDTreeCusMem<FPType, N, ElemType, DT>::KDTreeCusMem(ConstRAI cbegin,
                                                    ConstRAI cend)
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
KDTreeCusMem<FPType, N, ElemType, DT> &
KDTreeCusMem<FPType, N, ElemType, DT>::operator=(KDTreeCusMem &&rhs) &noexcept {
    if (this != &rhs) {
        root = rhs.root;
        treeSize = rhs.treeSize;
...
/*
template <typename FPType, std::uint8_t N, typename ElemType,
typename def::DistType DT> template <class RAI> void KDTreeCusMem<FPType, N,
ElemType, DT>:: RangeCtorHelper(TreeNode*& ndPoolPtr, RAI begin, RAI median, RAI
end, std::array<FPType, N>& bbox) { std::uint8_t dim =
std::max_element(bbox.begin(), bbox.end()) - bbox.begin();
std::nth_element(begin, median, end, [=](const auto& p1, const auto& p2) {
return p1.first[dim] < p2.first[dim];}); pool->construct(ndPoolPtr,
std::move(median->first), std::move(median->second)); auto cur_ndPtr =
ndPoolPtr;
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
template <class RAI>
void KDTreeCusMem<FPType, N, ElemType, DT>::RangeCtorHelper(
    TreeNode *&ndPoolPtr, std::uint8_t dim, RAI begin, RAI median, RAI end) {
...
KDTreeCusMem<FPType, N, ElemType, DT>::dimension() const {
    return N;
}
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
void KDTreeCusMem<FPType, N, ElemType, DT>::insert(const PointND<FPType, N> &pt,
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
bool KDTreeCusMem<FPType, N, ElemType, DT>::contains(
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType &KDTreeCusMem<FPType, N, ElemType, DT>::operator[](
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType &
KDTreeCusMem<FPType, N, ElemType, DT>::at(const PointND<FPType, N> &pt) {
    return const_cast<ElemType &>(
        static_cast<const KDTreeCusMem &>(*this).at(pt));
}

template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
const ElemType &
KDTreeCusMem<FPType, N, ElemType, DT>::at(const PointND<FPType, N> &pt) const {
...
KDTreeCusMem<FPType, N, ElemType, DT>::findNodePtr(
    const PointND<FPType, N> &pt) {
    return const_cast<TreeNode **>(
        static_cast<const KDTreeCusMem *>(this)->findNodePtr(pt));
}
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType
KDTreeCusMem<FPType, N, ElemType, DT>::kNNValue(const PointND<FPType, N> &pt,
                                                std::size_t k) const {
    if (k == 1)
        return NNValue(pt);
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
void KDTreeCusMem<FPType, N, ElemType, DT>::kNNValueHelper(
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
template <class OutputIter>
OutputIter KDTreeCusMem<FPType, N, ElemType, DT>::NNsWithFence(
    ...
    /*
     ...
     std::numeric_limits<FPType>::max(); NNsWithFenceHelper(root, 0, key, diff,
     distKVPairs, bestSq, bestDiffSq); for (const auto &p : distKVPairs) { if
     (p.first < bestDiffSq) *it++ = std::move(p.second);
     }
     return it;
     */
...
/*
 template <typename FPType, std::uint8_t N, typename ElemType,
 typename def::DistType DT> void KDTreeCusMem<FPType, N, ElemType, DT>::
 NNsWithFenceHelper(TreeNode *cur, std::uint8_t dim, const PointND<FPType, N>&
 pt, FPType diff, std::vector<std::pair<FPType, std::pair<PointND<FPType, N>,
 ElemType>>> &distKVPairs, FPType& bestDistSq, FPType&bestDistDiffSq) const {
 auto distSq = PointND<FPType, N>::template
 ...
 bestDistDiffSq); if (thisDiff*thisDiff < bestDistDiffSq) { next = next ==
 cur->left ? cur->right : cur->left; if (next) NNsWithFenceHelper(next,
 next_dim, pt, diff, distKVPairs, bestDistSq, bestDistDiffSq);
 }
 }*/

template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
ElemType KDTreeCusMem<FPType, N, ElemType, DT>::NNValue(
    const PointND<FPType, N> &pt) const {
    ...
    static constexpr std::size_t TREE_SIZE_LOWER_BOUND = 1200,
                                 TREE_SIZE_UPPER_BOUND = 600000000;
    logCondition =
        treeSize <= TREE_SIZE_UPPER_BOUND && treeSize >= TREE_SIZE_LOWER_BOUND;
    ...
    if (logCondition) {
        ...
        std::cout << "***** Log of treesize from " << TREE_SIZE_LOWER_BOUND
                  ...
                  << "\nTotal num of NN searches with this criteria: "
                  << numNNSearches << "\n\nTreesize: " << treeSize
                  << "\nNum of nodes searched in this NN search: "
                  << thisNumNodesSearches
...
void KDTreeCusMem<FPType, N, ElemType, DT>::NNValueHelper(
    TreeNode *cur, std::uint8_t dim, const PointND<FPType, N> &pt,
...
template <typename FPType, std::uint8_t N, typename ElemType,
          typename def::DistType DT>
FPType KDTreeCusMem<FPType, N, ElemType, DT>::branchMin(
