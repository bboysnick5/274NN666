/*
 * @Author: Nick Liu
 * @Date: 2021-06-13 18:35:34
 * @LastEditTime: 2022-07-31 12:00:44
 * @LastEditors: Nick Liu
 * @Description:
 * @FilePath: /274F201666/cppsrc/PoolAllocator.hpp
 */
...
class PooledAllocator {
  ...
  public:
    ...
    template <typename T> void static destroy(T *p) { p->~T(); }

    template <typename T> void destroy_and_free_all() {
        while (base) {
            std::destroy_n(reinterpret_cast<T *>(static_cast<char *>(base) +
                                                 sizeof(void *)),
                           (static_cast<char *>(loc) -
                            static_cast<char *>(base) + remaining) /
                               sizeof(T));
     ...
     */
    template <typename T> T *allocateExact(const std::size_t count = 1) {
        return static_cast<T *>(this->malloc(sizeof(T) * count));
    }

    template <typename T> T *allocatePool(const std::size_t count = 1) {
