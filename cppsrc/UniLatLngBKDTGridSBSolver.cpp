...
void UniLatLngBKDTGridSBSolver<KDTType, dist_type>::calcSideLenFromAlpc() {
    dist_type surfaceArea = 4*std::numbers::pi_v<dist_type>*SBLoc<dist_type>::EARTH_RADIUS*SBLoc<dist_type>::EARTH_RADIUS;
...
findNearest(const Point<dist_type, 2>& geoSearchPt) const {
    ...
    ((geoSearchPt[0]+0.5*std::numbers::pi_v<dist_type>)*latIncInverse)*colSize+ static_cast<size_t>((geoSearchPt[1]+std::numbers::pi_v<dist_type>)/lngInc)]);
}
