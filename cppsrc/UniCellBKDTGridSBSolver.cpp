...
void UniCellBKDTGridSBSolver<KDTType, dist_type>::fillGridCache() {
    ...
    for (size_t r = 0, idx = 0; r < this->rowSize;
        ...
        size_t thisColSize = static_cast<size_t>(2*std::numbers::pi_v<dist_type> * SBLoc<dist_type>::EARTH_RADIUS *
                             cos(thisLat > 0 ? thisLat-this->latInc : thisLat)/
                             this->sideLen) + 2;
        dist_type thisLngInc = 2*std::numbers::pi_v<dist_type>/thisColSize + 2*std::numbers::pi_v<dist_type>/(thisColSize*thisColSize*65536);
        ...
        dist_type diagonalDist3DEUC = SBLoc<dist_type>::EUC3DDistFromLatDeltaLng(lat1, lat1 + this->latInc, thisLngInc),
               thisCtrLng = 0.5 * thisLngInc - std::numbers::pi_v<dist_type>;
...
findNearest(const Point<dist_type, 2>& geoSearchPt) const {
    const auto &[startIdx, thisLngIncInverse] = thisRowStartIdx[(geoSearchPt[0]+0.5*std::numbers::pi_v<dist_type>)*this->latIncInverse];
    return UniLatLngBKDTGridSBSolver<KDTType, dist_type>::returnNNLocFromCacheVariant(geoSearchPt,
        this->gridCache[startIdx + static_cast<size_t>((geoSearchPt[1]+std::numbers::pi_v<dist_type>)*thisLngIncInverse)]);
}
